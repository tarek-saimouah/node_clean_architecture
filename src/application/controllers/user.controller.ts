...
const repository: IUserRepo = container.get<IUserRepo>(Types.IUserRepo)
const otpService: OtpService = OtpService.getInstance()
...
const findAllUsers = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const users = await repository.findPaging(query, offset, limit)

    const responseUsers = users.map((user) => generateUserResponse(user))
...
const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (updates.email) {
      const exists = await repository.isExists(updates.email, undefined, id)
    ...
    const updatedUser = await repository.updateOne({ ...updates, id })
...
const verifyAccount = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user.otpExpire || new Date() > new Date(user.otpExpire)) {
      ...
      await repository.updateOne(updates)
...
const resendOtpCode = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const otpResponse: SendOtpResponse =
      await otpService.sendVerificationMessage(user.phoneNumber, otpCode)
...
const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updated = await repository.updateOne(updates)
...
const deleteUserAccount = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await repository.findOne({ phoneNumber }, selectPassword)
...
// helper functions
const generateUserResponse = (user: UserEntity) => {
  return {
    ...user,
    password: undefined,
    otpCode: undefined,
