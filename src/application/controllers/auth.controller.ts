...
const signupUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userEmailExists = await userRepository.isExists(email)
    ...
    const userPhoneNumberExists = await userRepository.isExists(
      undefined,
      phoneNumber
    )
    ...
    const otpResponse: SendOtpResponse =
      await otpService.sendVerificationMessage(phoneNumber, otpCode)
...
const loginUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await userRepository.findOne({ phoneNumber }, selectPassword)
...
const forgotPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = await userRepository.findOne(query, false)
    ...
    const saved = await userRepository.updateOne({
      id: user.id,
      otpCode,
      otpExpire,
    })
